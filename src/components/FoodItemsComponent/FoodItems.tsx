...
const EditableCell: React.FC<EditableCellProps> = ({
  ...
  const inputNode =
    inputType === "number" ? (
      <InputNumber min={0} size="large" placeholder={`${title}`} />
    ) : (
...
export function FoodItemsComponent() {
  ...
  const SelectInput: React.FC<{ isDisabled: boolean; record: Item }> = ({
    isDisabled,
    record,
  }) => {
          ...
          setDataSource((prevDataSource) => {
            const newData = prevDataSource.map((item) =>
              item.key === record.key
                ? {
                    ...item,
                    image: { ...item.image, name, type, size, base64Url },
                  }
                : item
            );
            return newData;
    ...
    return (
      ...
      >
        {record.image.base64Url ? (
          <img src={record.image.base64Url} alt="Image" />
        ) : (
          uploadButton
        )}
      </Upload>
